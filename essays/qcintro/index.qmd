...
- Quantum Computing
description: >
    A sceptical perspective, version 0.1
# formatting
toc: true
toc-title: Contents
reading-time: true
...
If there is any justification for Yet Another Introduction to Quantum Computing, it is that this one tries to avoid the mystification that often comes with discussion of quantum phenomena.

This introduction assumes no knowledge of quantum computing, but does assume some familiarity with general (classical) computing.

Here are some questions I hope to answer:

-   How are quantum computers built? In particular, are we going to have quantum laptops any time soon?
...
-   What advantages, if any, does quantum computing offer? This topic is skipped over very lightly here, but a Part II will discuss it in more depth (but still not deeply).
...
Let's start with the basics, though, which is the idea of a qubit...

# Qubits

You have probably heard that whereas regular computers are built around bits, quantum computers are built around qubits. So what does that mean?

The archetypal example of a qubit is an atomic nucleus with a spin of ½, like the ^89^Y isotope of Yttrium. The spin means that the nucleus acts as if it were an electrical charge spinning in one direction or the other, and this spinning electrical charge creates a little magnet that can be pointing either “up” or “down”.

It's a quantum property, which you just have to get used to, that the nucleus cannot spin at any speed, but only at discrete values (hence "quanta"), so that any measurement of the spin of the nucleus along a given axis gives either the value +½ (up) or -½ (down). These two values can represent $0$ and $1$, just as a classical "bit" might be a transistor that can take on a state of "on" or "off".

You may have heard that qubits can exist in combinations of 0 and 1, but let's leave that to one side for now.
...
But once you realise that a circuit is not a permanent thing but is instead a sequence of operations (gates) that implement an algorithm, then it’s not a contradiction at all.
...
If you prepare a qubit in state $| \uparrow \rangle$ then there is a gate called the Hadamard gate that can put it into a superposition, with $\alpha$ and $\beta$ both $1/\sqrt{2}$. Many circuits start with Hadamard gates way to take advantage of this superposition. See

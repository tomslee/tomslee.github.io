---
title: High Emissions and Low Pay 
subtitle: Uber is Still Taking Regulators for a Ride
author: Tom Slee
date: 2023-01-30
categories:
  - Essays
...
A report for [RideFairTO](https://ridefair.ca/).

::: callout-note
You can also read this essay as a [PDF](ridehail.pdf)
:::
...
![A City in the simulation is simply a square, with C blocks on each side.](fig1.png){#fig-1 width=70%}
...
![The red dots show the origin of trips in a small "two-zone" city if eight blocks. The central area has a higher demand than outlying suburbs.](fig3.png){#fig-3 width=70%}
...
![Simulating Toronto: Wait times and vehicle phases for Feb 6, 2020. The figures at 6000 vehicles match the reported values for that day.](fig4.png){#fig-4 width=70%}
...
![The Australian Cycling Promotion Fund is one of several groups to have produced photos like this, showing the relative space efficiency of different modes of transport. This is reproduced from the Human Transit blog of Jarrett Walker.](fig5.jpg){#fig-5 width=70%}
...
![Uber's losses over the last 7 years.](fig6.png){#fig-6 width=70%}
...
Other cities, with New York City and San Francisco as outliers, have utilization rates (by time) of 50 to 60%.[^ridehail-1] Toronto has a level of demand that is lower than Chicago, which may provide a reasonable upper bound to what can be expected of around 60%.

[^ridehail-1]: Utilization rates measured by distance may be slightly larger, as drivers drive slower when not engaged in a trip.
...
![The number of vehicles N and utilization rate P3 required to support a wait time of 6 minutes (w=0.30).  While the utilization rate (P3) does increase with demand, the improvements level off as demand increases.  Meanwhile, the number of vehicles required keeps growing (green line, at left).](fig7.png){#fig-7 width=70%}
...
Drivers also have costs. These include fixed costs c~*f*~ (such as the purchase of a car), and marginal cost of operations *c~o~* (fuel, insurance, maintenance, cleaning, etc.).[^ridehail-2] Revenue Canada gives a "reasonable per-kilometre allowance" of 55c per kilometre to account for fuel, insurance, maintenance and depreciation. Although ridehail drivers may have additional costs as a result of commercial insurance, additional cleaning fees, and driving predominantly in a low-speed urban environment, we adopt this figure.
...
![ In Chicago, full-time drivers (defined as those who drive over 275 trips per month), make up only about one in six drivers, but are responsible for over 40% of trips.](fig8.png){#fig-8 width=70%}
...
![ "Before" and "After" a change in fare. While the change affects the number of vehicles and passenger wait times, driver income does not change.](fig9.png){#fig-9 width=70%}
